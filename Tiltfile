...
k8s_resource('webapp', port_forwards=[3000], resource_deps=[], labels=["apps"])

# Adds the dotenv secrets resource
k8s_resource(new_name='dotenv', labels=["secrets"], objects=['dotenv:secret'])

# Database
k8s_resource(new_name='db', port_forwards=[5432], objects=['db:Kubegres:default'], labels=["infra"])

# Queue
k8s_resource(new_name='queue', port_forwards=[15672, 5672], objects=['queue:rabbitmqcluster:default'], labels=["infra"])

# Worker
k8s_resource('worker-ping', labels=['worker'])

k8s_resource(new_name='worker-ping-scaler', objects=['worker-ping-scaler:ScaledObject:default'], labels=['keda-scaler'])
